    ...
    parse (JSONString){
        try {
            ...
            for (var i = 0; i < category_count; i++){
                ...
                tempRates[rarity_name][category_name] = parseItemList(rawRates['appear'][i], rawRates['appear'][i]['category'] == 0);
            }
...
function parseItemList(rate_list, isCharacterList){
    ...
    for (var j = 0; j < item_count; j++){
        ...
        if(isCharacterList){
            cleaned_item['character_name'] = item['character_name'];
        }
    ...
    for (var key in item_list){
        item_list[key]['count'] = item_list[key]['list'].length;
        item_list[key]['total_rate'] = item_list[key]['list'].length * parseFloat(key) * 1000;
    }
