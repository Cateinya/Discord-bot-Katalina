...
const command: SlashCommand = {
    command: new SlashCommandBuilder()
        .setName("roll")
        .setDescription("Performs a Premium draw")
        ...
        try {
            ...
            if (id_or_alias) {
                ...
                if (id_regex.test(id_or_alias) && getRatesData(id_or_alias)) {
                    ratesID = id_or_alias;
                } else if (ratesAliasesServer && ratesAliasesServer[id_or_alias]) {
            ...
            const draws = drawMultiple(ratesID, number, ssr_only);
            ...
            const msg = formatDrawResults(draws, (!ssr_only), ratesID, ratesAliases, info);
